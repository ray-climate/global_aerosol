#!/usr/bin/env python
# -*- coding:utf-8 -*-
# @Filename:    plot_PSC_observation_number.py
# @Author:      Dr. Rui Song
# @Email:       rui.song@physics.ox.ac.uk
# @Time:        04/11/2023 00:50
        ...
        with open(os.path.join(INPUT_PATH, file), 'r') as f:
            ...
            for row in reader:
                try:
                    ...
                    if (SOUTHERN_LATITUDE <= latitude <= NORTHERN_LATITUDE) and (MIN_ALTITUDE <= alt_base <= MAX_ALTITUDE) and (MIN_ALTITUDE <= alt_top <= MAX_ALTITUDE) and (aerosol_type == 1.):
...
fig.suptitle('2011 Puyehue: Number of PSC Layers between 9 and 16 km', fontsize=20)  # Main title
...
# Save the figure
plt.savefig(os.path.join(FIGURE_OUTPUT_PATH, 'valid_PSC_values_by_latitude.png'))
