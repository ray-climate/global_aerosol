#!/usr/bin/env python
# -*- coding:utf-8 -*-
# @Filename:    plot_aerosol_mean_altitude.py
# @Author:      Dr. Rui Song
# @Email:       rui.song@physics.ox.ac.uk
# @Time:        06/11/2023 22:25
...
aerosol_layer_height_data = defaultdict(lambda: {'-20_to_-40': [], '-40_to_-60': [], '-60_to_-80': []})
            ...
            for row in reader:
                ...
                try:
                    ...
                    if (SOUTHERN_LATITUDE <= latitude <= NORTHERN_LATITUDE) and (MIN_ALTITUDE <= alt_base <= MAX_ALTITUDE) and (MIN_ALTITUDE <= alt_top <= MAX_ALTITUDE) and (2. <= aerosol_type <= 4.) & (abs(CAD) > 40):
                        if -40 < latitude <= -20:
                            aerosol_layer_height_data[aligned_date]['-20_to_-40'].append((alt_top + alt_base) / 2.)
                        elif -60 < latitude <= -40:
                            aerosol_layer_height_data[aligned_date]['-40_to_-60'].append((alt_top + alt_base) / 2.)
                        elif -80 < latitude <= -60:
                            aerosol_layer_height_data[aligned_date]['-60_to_-80'].append((alt_top + alt_base) / 2.)
                except:
                    pass
...
for date in sorted(aerosol_layer_height_data.keys()):
    for lat_range in aerosol_layer_height_data[date]:
        if len(aerosol_layer_height_data[date][lat_range]) > 10:  # Check if more than 10 data points are present
            mean_height = np.mean(aerosol_layer_height_data[date][lat_range])
            std_height = np.std(aerosol_layer_height_data[date][lat_range])
            plot_data[lat_range]['dates'].append(date)
            plot_data[lat_range]['means'].append(mean_height)
            plot_data[lat_range]['stds'].append(std_height)
...
for i, (lat_range, color) in enumerate(colors.items()):
    ...
    if len(plot_data[lat_range]['dates']) > 0:  # This ensures the array is not empty
        axs[i].errorbar(
            ...
            color=color, label='Aerosol Layer Height', linestyle='none'
        )
...
# Save the figure
plt.savefig(os.path.join(FIGURE_OUTPUT_PATH, 'aerosol_layer_height_over_time_subplot.png'))
