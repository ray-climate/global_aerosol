#!/usr/bin/env python
# -*- coding:utf-8 -*-
# @Filename:    caliop_extraction_v2.py
# @Author:      Dr. Rui Song
# @Email:       rui.song@physics.ox.ac.uk
# @Time:        15/06/2023 14:55
...
save_location = './caliop_ash_data_extraction_extended_v1'
...
for caliop_sub_folder in os.listdir(caliop_location + '/' + year):
    ...
    for files in os.listdir(caliop_location + '/' + year + '/' + caliop_sub_folder):
        if files.endswith('.hdf'):
            ...
            backscatter = request._get_calipso_data(filename=caliop_file, variable='Total_Backscatter_Coefficient_532')
            depolarisation = request._get_calipso_data(filename=caliop_file, variable='Particulate_Depolarization_Ratio_Profile_532')
