...
all_data = pd.DataFrame(columns=['thickness', 'utc_time', 'latitude'])

for file in files:
    data = pd.read_csv(variable_file_location + '/' + file)
    print(f"Processing file {file}")

    # Parse the utc_time to datetime format
    data['utc_time'] = pd.to_datetime(data['utc_time'])

    for column in ['thickness', 'latitude']:
        data[column] = pd.to_numeric(data[column], errors='coerce')

    all_data = all_data.append(data[['thickness', 'utc_time', 'latitude']], ignore_index=True)
...
grouped_data = all_data.groupby(['utc_time', 'latitude_bin']).mean().reset_index()
...
c = ax.imshow(pivoted_data, aspect='auto', cmap='rainbow', origin='lower', vmin=0, vmax=4.)
# Add utc_time line plot

# ax.set_title('Average Thickness vs Latitude and Time', fontsize=20)
ax.set_xlabel('Latitude [°]', fontsize=18)
ax.set_ylabel('Time [UTC]', fontsize=18)
ax.set_xlim(-80, 80)
