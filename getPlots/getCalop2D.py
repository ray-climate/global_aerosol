    ...
    for i in range(len(index)):
        for j in range(1, len(alt)-1):
            try:
                ...
                beta2D_proj[(altitude_grid_regular <= np.max(alt_range)) & (altitude_grid_regular >= np.min(alt_range)) & (longitude_grid_regular <= np.max(lon_range)) & (longitude_grid_regular >= np.min(lon_range))] = beta[[j], index[i]]
            except:
                pass

    # create a plot to show the 2D beta field
