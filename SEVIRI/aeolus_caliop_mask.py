...
def get_caliop_mask(SEVIRI_HR_file_path, BTD_ref, extent, title, save_str,
    ...
    coords = np.stack((seviri_lats.ravel(), seviri_lons.ravel()), axis=-1)
    tree = cKDTree(coords)
    search_points = np.hstack((caliop_lat.reshape(caliop_lat.size, 1), caliop_lon.reshape(caliop_lon.size, 1)))
    distances, indices = tree.query(search_points)

    caliop_cm_values = dust_mask.ravel()[indices]
    caliop_mask[caliop_cm_values == 1.] = 1.
    ...
    ax.scatter(caliop_lon, caliop_lat, marker='o', color='red', s=10, transform=CRS, zorder=200,
               label='Caliop')
    ax.scatter(caliop_lon[caliop_mask==1.], caliop_lat[caliop_mask==1.], marker='P', color='green', s=20, transform=CRS, zorder=300)
