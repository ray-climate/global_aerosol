...
plt.plot(lr_caliop_mean, alt_caliop, linestyle='-', color='green', linewidth=3, alpha=0.6, label='CALIOP 532 nm')
...
for i in range(len(lr_aeolus_mean) - 1):
    ...
    if i == 0:
        plt.plot([lr_aeolus_mean[i], lr_aeolus_mean[i]], [alt_aeolus_mean[i], alt_aeolus_mean[i + 1]], linestyle='-', color='blue', linewidth=3, label='ALADIN 355 nm')
    else:
