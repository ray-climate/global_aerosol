...
caliop_alpha_low_aod_mean = np.nanmean(caliop_alpha_low_aod, axis=0)
caliop_alpha_high_aod_mean = np.nanmean(caliop_alpha_high_aod, axis=0)
...
caliop_aod_low_upper = np.trapz(caliop_alpha_low_aod_mean[::-1][caliop_altitude>2.4], caliop_altitude[::-1][caliop_altitude>2.4])
caliop_aod_low_bottom = np.trapz(caliop_alpha_low_aod_mean[::-1][caliop_altitude<2.4], caliop_altitude[::-1][caliop_altitude<2.4])
caliop_aod_high_upper = np.trapz(caliop_alpha_high_aod_mean[::-1][caliop_altitude>2.4], caliop_altitude[::-1][caliop_altitude>2.4])
caliop_aod_high_bottom = np.trapz(caliop_alpha_high_aod_mean[::-1][caliop_altitude<2.4], caliop_altitude[::-1][caliop_altitude<2.4])
...
ax.tick_params(axis='both', labelsize=fontsize)
ax.legend(loc='best', fontsize=fontsize)
plt.savefig(save_path + f'figure_extinction_low.png', dpi=300)
plt.close('all')

fig, ax = plt.subplots(figsize=(8, 12))
plt.plot(caliop_alpha_high_aod_mean, caliop_altitude, 'k-', lw=3, label='CALIOP')
plt.fill_betweenx(caliop_altitude, caliop_alpha_high_aod_mean - caliop_alpha_high_aod_std, caliop_alpha_high_aod_mean + caliop_alpha_high_aod_std, color='gray', alpha=0.4)
ax.set_ylabel('Altitude [km]', fontsize=fontsize)
ax.set_xlabel('Extinction [km$^{-1}$]', fontsize=fontsize)
ax.set_ylim(0, 10.)
ax.set_xlim(1.e-2, 1.e1)
...
ax.tick_params(axis='both', labelsize=fontsize)
ax.legend(loc='best', fontsize=fontsize)
plt.savefig(save_path + f'figure_extinction_high.png', dpi=300)
plt.close('all')
