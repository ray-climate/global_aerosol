...
for i in range(len(resampled_timestamps)):
    smoothed_aod_data[:, i] = gaussian_filter(resampled_aod_data[:, i], sigma=15)
...
ax.set_title('AOD at aerosol layer %s - %s km [%d$^{\circ}$W - %d$^{\circ}$W]' % (alt_1, alt_2, abs(lon1), abs(lon2)), fontsize=16, pad=15)
...
# Set the colorbar labels
cbar = fig.colorbar(cm.ScalarMappable(cmap=cmap, norm=mcolors.Normalize(vmin=0, vmax=1)), cax=cax2, orientation='vertical', ticks=[0, 1])
